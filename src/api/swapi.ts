...
export const fetchPeople = async (): Promise<Person[]> => {
  ...
  const withImageSrc: Person[] = await Promise.all(
    data.map(async (person) => {
      ...
      try {
        const { data: imageData }: { data: { image: string } } = await axios.get(`${IMAGE_BASE}/${id}.json`);
...
export const fetchPerson = async (id: string): Promise<Person> => {
  const { data }: { data: Person } = await axios.get(`${SWAPI_BASE}/people/${id}`);
  return data;
};
...
export const fetchPlanet = async (id: string): Promise<Planet> => {
  const { data }: { data: Planet } = await axios.get(`${SWAPI_BASE}/planets/${id}`);
  return data;
};

export const fetchStarships = async (): Promise<Starship[]> => {
  const { data }: { data: Starship[] } = await axios.get(`${SWAPI_BASE}/starships`);
  return data;
};

export const fetchStarship = async (id: string): Promise<Starship> => {
  const { data }: { data: Starship } = await axios.get(`${SWAPI_BASE}/starships/${id}`);
  return data;
};
